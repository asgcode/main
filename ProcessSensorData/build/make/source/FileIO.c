                    ...
                    FILE* hFile) ///< [in] File handle in which number of entries needs to be looked up
{
    ...
    return CONVERT_BYTES_TO_ENTRIES(fileSizeInBytes);
}
              ...
              CircularBuffer* pOutCBuf)            //< [out] Output circular buffer in which data was read
{
    ...
    readBytes   = fread(pOutCBuf->pData, sizeof(UCHAR), CONVERT_ENTRIES_TO_BYTE(numEnriesToBeRead), hFile);
    readEntries = CONVERT_BYTES_TO_ENTRIES(readBytes);
